export interface CameraShakeOptions {
    ...
    amplitude?: number | BABYLON.Vector3
    frequency?: number | BABYLON.Vector3
    frequencyOffset?: number

    rotation?: number | BABYLON.Vector3
    rotationFrequency?: number | BABYLON.Vector3
    ...
    onBeforeUpdate?: (
        targetValues: Pick<
            CameraShakeOptions,
            'amplitude' | 'frequency' | 'rotation' | 'rotationFrequency'
        >
    ) => Pick<
        CameraShakeOptions,
        'amplitude' | 'frequency' | 'rotation' | 'rotationFrequency'
...
const defaultCameraShakeOptions: Required<
    Omit<CameraShakeOptions, 'onBeforeUpdate'>
> = {
    shakePattern: 'perlin',
...
export class CameraShake implements Behavior<TransformNode> {
    ...
    public attach(target: TransformNode): void {
        ...
        this.beforeRenderObserver = this.target
            .getScene()
    ...
    public detach(): void {
        ...
        if (this.target && this.beforeRenderObserver) {
            this.target
                .getScene()
                .onBeforeRenderObservable.remove(this.beforeRenderObserver)
    ...
    private shake(): void {
        ...
        const shakePatternFunc =
            this.options.shakePattern === 'perlin'
    ...
    private shakePosition(
        time: number,
        shakePatternFunc: (x: number) => number
    ): void {
        if (!this.target || !this.parent) {
            return
        }
        ...
        const frequencyOffset =
            this.options.frequencyOffset ||
            defaultCameraShakeOptions.frequencyOffset ||
            0
        const influence =
            this.options.influence ?? defaultCameraShakeOptions.influence ?? 1

        const x =
            amplitude.x *
            (shakePatternFunc((frequency.x + frequencyOffset) * time) * 0.5 +
                0.5) *
            influence
        const y =
            amplitude.y *
            (shakePatternFunc((frequency.y + frequencyOffset) * time) * 0.5 +
                0.5) *
            influence
        const z =
            amplitude.z *
            (shakePatternFunc((frequency.z + frequencyOffset) * time) * 0.5 +
                0.5) *
            influence
    ...
    private shakeRotation(
        ...
        const frequency = this.getValueAsVector(this.options.frequency)
        const rotationFrequency = this.getValueAsVector(
            this.options.rotationFrequency
        )
        const rotationOffset =
            this.options.rotationOffset ||
            defaultCameraShakeOptions.rotationOffset ||
            0
        const influence =
            this.options.influence ?? defaultCameraShakeOptions.influence ?? 1
        ...
        const x =
            rotation.x *
            shakePatternFunc(
                (frequency.x + rotationFrequency.x + rotationOffset) * time
            ) *
            influence
        const y =
            rotation.y *
            shakePatternFunc(
                (frequency.y + rotationFrequency.y + rotationOffset) * time
            ) *
            influence
        const z =
            rotation.z *
            shakePatternFunc(
                (frequency.z + rotationFrequency.z + rotationOffset) * time
            ) *
            influence
        ...
        const finalQuaternion = quaternionX
